...
# Runtime latch: once OpenRouter answers 429 (free-tier daily cap exhausted),
# every later call in this process routes to Groq until restart. Free-tier
# limits only reset daily, so retrying OpenRouter mid-process never helps.
_FORCE_GROQ = False
_log = logging.getLogger(__name__)


def force_groq_fallback(reason: str = "") -> bool:
    """Latch all subsequent LLM routing onto Groq.

    Args:
        reason: Human-readable cause recorded in the log (e.g. the 429 body).

    Returns:
        ``True`` when this call flipped the latch from off to on.
    """
    global _FORCE_GROQ
    if _FORCE_GROQ:
        return False
    _FORCE_GROQ = True
    _log.warning(
        "OpenRouter unavailable (%s) - latching all LLM routing onto Groq "
...
def is_groq_forced() -> bool:
    """Whether routing has been latched onto Groq for this process."""
    return _FORCE_GROQ
...
def get_llm_config(task_type: str) -> dict[str, str]:
    ...
    """
    # 429 latch: route everything through Groq once OpenRouter is capped.
    if _FORCE_GROQ and GROQ_API_KEY:
        quick = {"scrape", "quick"}
        model = _GROQ_FALLBACK_QUICK if task_type in quick else _GROQ_FALLBACK_DEEP
        return {
            "model": model,
